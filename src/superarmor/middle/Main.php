...
use superarmor\middle\Data\{Enchantments, Items};
		...
		if ($cmd->getName() == "armor") {
			...
			$form = new SimpleForm(
				function (Player $sender, ? int $result = null) {
					...
					switch ($result) {
						case 0:
							$item_id = $inventory->getItemInHand()->getId(); //переменная для проверки id предмета в руке игрока
							...
							$inventory->removeItem(
								ItemFactory::getInstance()->get($this->config->getNested("buy-armor.id-item-for-buy"), 0, $this->config->getNested("full-armor.cost"))
							);
						...
						case 1:
							$sender->sendMessage(
								"После покупки вам будет выдана броня " .
								 $this->config->getNested("leggins.enchants.level") . 
								" защиты\n Стоимость этой брони " . 
								$this->config->getNested("full-armor.cost") . " алмазов"
							);
							break;
